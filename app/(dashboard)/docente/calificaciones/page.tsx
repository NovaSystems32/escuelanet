...
export default function DocenteCalificacionesPage() {
  ...
  const getNotaColor = (nota: number) => nota >= 7 ? 'bg-green-100 text-green-700' : nota >= 4 ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700';
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                ...
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 uppercase">Trim.</th>
              ...
              {filtered.map(c => {
                ...
                return (
                  <tr key={c.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 text-sm text-slate-600">{tipoOpts.find(t => t.value === c.tipo)?.label}</td>
      ...
      <Modal isOpen={isOpen} onClose={() => setIsOpen(false)} title={editId ? 'Editar Calificación' : 'Nueva Calificación'} size="lg">
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <select className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" value={form.materiaId} onChange={e => setForm(f => ({ ...f, materiaId: e.target.value, estudianteId: '' }))}>
              {misMaterias.map(m => <option key={m.id} value={m.id}>{m.nombre}</option>)}
            </select>
          </div>
          <div>
