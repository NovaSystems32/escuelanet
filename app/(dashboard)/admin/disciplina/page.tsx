...
const tipoConfig: Record<string, { label: string; color: string }> = {
  ...
  suspension: { label: 'Suspensión', color: 'bg-red-100 text-[#e53935]' },
  felicitacion: { label: 'Felicitación', color: 'bg-[#dcfce7] text-[#15803d]' },
};
    ...
    <div>
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        {['', 'observacion', 'apercibimiento', 'suspension', 'felicitacion'].map(tipo => (
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-sm font-medium transition-colors ${filterTipo === tipo ? 'bg-[#2d4a8a] text-white' : 'bg-white border border-[#d8e0ee] text-[#5a6a8a] hover:bg-[#f8f9fc]'}`}
          >
      ...
      <div className="space-y-3">
        {filtered.map(reg => {
          ...
          return (
            <div key={reg.id} className="bg-white rounded-xl border border-[#d8e0ee] p-5">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2 flex-wrap">
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${config.color}`}>{config.label}</span>
                    {reg.resuelto && <span className="text-xs px-2 py-0.5 rounded-full bg-[#eef1f8] text-[#5a6a8a]">Resuelto</span>}
                    <span className="text-xs text-[#5a6a8a]">{new Date(reg.fecha).toLocaleDateString('es-AR')}</span>
                  </div>
                  <p className="text-sm font-medium text-[#1a2444] mb-1">{est ? `${est.apellido}, ${est.nombre}` : '-'}</p>
                  <p className="text-sm text-[#5a6a8a]">{reg.descripcion}</p>
                  {docente && <p className="text-xs text-[#5a6a8a] mt-1">Por: Prof. {docente.apellido}</p>}
                </div>
