          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-[#e8e8ec]">
              {materias.map(m => {
                ...
                return (
                  <tr key={m.id} className="hover:bg-[#f4f4f6]">
                    <td className="px-4 py-3">
                      <p className="text-sm font-medium text-[#111111]">{m.nombre}</p>
                      {m.descripcion && <p className="text-xs text-[#888888] truncate max-w-xs">{m.descripcion}</p>}
                    </td>
                    <td className="px-4 py-3 text-sm text-[#888888]">{curso ? `${curso.nombre} ${curso.division}` : '-'}</td>
                    <td className="px-4 py-3 text-sm text-[#888888]">{docente ? `${docente.apellido}, ${docente.nombre}` : '-'}</td>
                    <td className="px-4 py-3 text-sm text-[#888888]">{m.horasSemanal}h</td>
                    <td className="px-4 py-3 text-right">
                      <button onClick={() => openEdit(m)} className="text-xs text-[#1a5276] hover:underline mr-3">Editar</button>
                      <button onClick={() => deleteMateria(m.id)} className="text-xs text-[#c62828] hover:underline">Eliminar</button>
                    </td>
                  </tr>
                );
              })}
      ...
      <Modal isOpen={isOpen} onClose={() => setIsOpen(false)} title={editId ? 'Editar Materia' : 'Nueva Materia'}>
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-[#111111] mb-1">Horas semanales</label>
              <input type="number" min={1} max={10} className="w-full px-3 py-2 border border-[#e8e8ec] rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1a5276]" value={form.horasSemanal} onChange={e => setForm(f => ({ ...f, horasSemanal: Number(e.target.value) }))} />
            </div>
          </div>
          <div>
            <label className="block text-sm font-medium text-[#111111] mb-1">Docente</label>
