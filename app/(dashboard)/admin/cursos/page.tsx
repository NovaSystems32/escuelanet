    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {cursos.map(curso => {
          ...
          return (
            <div key={curso.id} className="bg-white rounded-xl border border-slate-200 p-5">
              <div className="flex items-start justify-between mb-3">
                <div>
                  <h3 className="font-semibold text-slate-900">{curso.nombre} - Div. {curso.division}</h3>
                  <p className="text-xs text-slate-500">{curso.nivel} • Turno {curso.turno}</p>
                </div>
                <div className="flex gap-2">
                  <button onClick={() => openEdit(curso)} className="text-xs text-blue-600 hover:underline">Editar</button>
                  <button onClick={() => deleteCurso(curso.id)} className="text-xs text-red-600 hover:underline">Eliminar</button>
                </div>
              </div>
              <div className="space-y-2">
                <div className="flex items-center justify-between text-sm">
                  <span className="text-slate-500">Docente a cargo:</span>
                  <span className="text-slate-800 font-medium">{docente ? `Prof. ${docente.apellido}` : 'Sin asignar'}</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-slate-500">Estudiantes:</span>
                  <span className="text-slate-800 font-medium">{estudiantesCurso.length}</span>
                </div>
              </div>
              <div className="mt-3 flex flex-wrap gap-1">
                {estudiantesCurso.slice(0, 3).map(e => (
                  <span key={e.id} className="text-xs bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full">{e.nombre} {e.apellido}</span>
                ))}
                {estudiantesCurso.length > 3 && (
                  <span className="text-xs bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full">+{estudiantesCurso.length - 3}</span>
                )}
              </div>
            </div>
          );
      ...
      <Modal isOpen={isOpen} onClose={() => setIsOpen(false)} title={editId ? 'Editar Curso' : 'Nuevo Curso'}>
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" value={form.nombre} onChange={e => setForm(f => ({ ...f, nombre: e.target.value }))} placeholder="ej: 1er Año" />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">División</label>
            <input className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" value={form.division} onChange={e => setForm(f => ({ ...f, division: e.target.value }))} placeholder="A" />
          </div>
          <div>
          ...
          <div>
            ...
            <input className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" value={form.nivel} onChange={e => setForm(f => ({ ...f, nivel: e.target.value }))} />
          </div>
          <div className="col-span-2">
            <label className="block text-sm font-medium text-slate-700 mb-1">Docente a cargo</label>
