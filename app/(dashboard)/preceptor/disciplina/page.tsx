...
const tipoConfig: Record<string, { label: string; color: string; icon: string }> = {
  ...
  suspension: { label: 'Suspensión', color: 'bg-red-100 text-[#c62828]', icon: '🚫' },
  felicitacion: { label: 'Felicitación', color: 'bg-[#d4edda] text-[#155724]', icon: '⭐' },
};
  ...
  return (
    <div>
      ...
      <div className="flex gap-3 mb-4 flex-wrap">
        ...
        <select value={filterResuelto} onChange={e => setFilterResuelto(e.target.value as typeof filterResuelto)} className="px-3 py-2 border border-[#e8e8ec] rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1a5276]">
      ...
      <div className="space-y-3">
        {filtered.map(reg => {
          ...
          return (
            <div key={reg.id} className={`bg-white rounded-xl border border-[#e8e8ec] p-5 ${reg.resuelto ? 'opacity-70' : ''}`}>
              <div className="flex items-start justify-between">
                <div className="flex items-start gap-3 flex-1">
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      ...
                      {reg.resuelto
                        ? <span className="text-xs px-2 py-0.5 rounded-full bg-[#e8e8ec] text-[#888888]">✓ Resuelto</span>
                        : <span className="text-xs px-2 py-0.5 rounded-full bg-red-50 text-red-500">Pendiente</span>
                      }
                    </div>
                    <p className="text-sm font-semibold text-[#111111]">{est ? `${est.apellido}, ${est.nombre}` : '-'}</p>
                    <p className="text-sm text-[#888888] mt-1">{reg.descripcion}</p>
                    <p className="text-xs text-[#888888] mt-1">{new Date(reg.fecha).toLocaleDateString('es-AR', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
                  </div>
                </div>
                <div className="flex gap-2 ml-4 flex-col items-end">
                  ...
                  <button
                    ...
                    className={`text-xs px-2 py-1 rounded-lg border transition-colors ${reg.resuelto ? 'border-[#e8e8ec] text-[#888888] hover:bg-[#f4f4f6]' : 'border-green-300 text-[#155724] hover:bg-green-50'}`}
                  >
